...
script("$Xg.onclick = function () { return confirm('".($Ee?js_escape($Ee):'Opravdu?')."'); };","");}function
...
upload_error($n){$Be=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($n?'NepodaÅ™ilo se nahrÃ¡t soubor.'.($Be?" ".sprintf('MaximÃ¡lnÃ­ povolenÃ¡ velikost souboru je %sB.',$Be):""):'Soubor neexistuje.');}function
...
edit_form($a,$p,$J,$_i){global$b,$x,$di,$n;$Eh=$b->tableName(table_status1($a,true));page_header(($_i?'Upravit':'VloÅ¾it'),$n,array("select"=>array($a,$Eh)),$Eh);if($J===false)echo"<p class='error'>".'Å½Ã¡dnÃ© Å™Ã¡dky.'."\n";echo'<form action="" method="post" enctype="multipart/form-data" id="form">
...
lang($ii,$Ye=null){if(is_array($ii)){$Vf=($Ye==1?0:($Ye&&$Ye<5?1:2));$ii=$ii[$Vf];}$ii=str_replace("%d","%s",$ii);$Ye=format_number($Ye);return
...
h($f->error);}function
check_sqlite_name($C){global$f;$Jc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Jc)\$~",$C)){$f->error=sprintf('ProsÃ­m pouÅ¾ijte jednu z koncovek %s.',str_replace("|",", ",$Jc));return
false;}return
true;}function
create_database($l,$ob){global$f;if(file_exists($l)){$f->error='Soubor existuje.';return
...
as$l){if(!@unlink($l)){$f->error='Soubor existuje.';return
false;}}return
true;}function
rename_database($C,$ob){global$f;if(!check_sqlite_name($C))return
false;$f->__construct(":memory:");$f->error='Soubor existuje.';return@rename(DB,$C);}function
...
Min_DB;$i=$b->credentials();if($f->connect($i[0],$i[1],$i[2])){if(min_version(9,0,$f)){$f->query("SET application_name = 'Adminer'");if(min_version(9.2,0,$f)){$xh['Å˜etÄ›zce'][]="json";$si["json"]=4294967295;if(min_version(9.4,0,$f)){$xh['Å˜etÄ›zce'][]="jsonb";$si["jsonb"]=4294967295;}}}return$f;}return$f->error;}function
get_databases(){return
...
set_schema($Rg){global$f,$si,$xh;$I=$f->query("SET search_path TO ".idf_escape($Rg));foreach(types()as$U){if(!isset($si[$U])){$si[$U]=0;$xh['UÅ¾ivatelskÃ© typy'][]=$U;}}return$I;}function
...
unconvert_field($o,$I){return$I;}function
support($Oc){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Oc);}$x="oracle";$si=array();$xh=array();foreach(array('ÄŒÃ­sla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum a Äas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Å˜etÄ›zce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'BinÃ¡rnÃ­'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$y=>$X){$si+=$X;$xh[$y]=array_keys($X);}$zi=array();$mf=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$id=array("length","lower","round","upper");$od=array("avg","count","count distinct","max","min","sum");$lc=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$dc["mssql"]="MS SQL (beta)";if(isset($_GET["mssql"])){$Yf=array("SQLSRV","MSSQL","PDO_DBLIB");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
last_id(){global$f;return$f->last_id;}$x="elastic";$mf=array("=","query");$id=array();$od=array();$lc=array(array("json"));$si=array();$xh=array();foreach(array('ÄŒÃ­sla'=>array("long"=>3,"integer"=>5,"short"=>8,"byte"=>10,"double"=>20,"float"=>66,"half_float"=>12,"scaled_float"=>21),'Datum a Äas'=>array("date"=>10),'Å˜etÄ›zce'=>array("string"=>65535,"text"=>65535),'BinÃ¡rnÃ­'=>array("binary"=>255),)as$y=>$X){$si+=$X;$xh[$y]=array_keys($X);}}$dc=array("server"=>"MySQL")+$dc;if(!defined("DRIVER")){$Yf=array("MySQLi","MySQL","PDO_MySQL");define("DRIVER","server");if(extension_loaded("mysqli")){class
Min_DB
extends
...
',script("focus(qs('#username'));"),"<p><input type='submit' value='".'PÅ™ihlÃ¡sit se'."'>\n",checkbox("auth[permanent]",1,$_COOKIE["adminer_permanent"],'TrvalÃ© pÅ™ihlÃ¡Å¡enÃ­')."\n";}function
login($re,$F){global$x;if($x=="sqlite")return
sprintf('Pro pÅ™ihlÃ¡Å¡enÃ­ k SQLite <a href="https://www.adminer.org/cs/extension/"%s>implementujte</a> metodu %s.',target_blank(),'<code>login()</code>');return
true;}function
tableName($Dh){return
h($Dh["Name"]);}function
fieldName($o,$rf=0){return'<span title="'.h($o["full_type"]).'">'.h($o["field"]).'</span>';}function
selectLinks($Dh,$O=""){global$x,$m;echo'<p class="links">';$qe=array("select"=>'Vypsat data');if(support("table")||support("indexes"))$qe["table"]='Zobrazit strukturu';if(support("table")){if(is_view($Dh))$qe["view"]='PozmÄ›nit pohled';else$qe["create"]='PozmÄ›nit tabulku';}if($O!==null)$qe["edit"]='NovÃ¡ poloÅ¾ka';$C=$Dh["Name"];foreach($qe
...
as$o){echo"<tr".odd()."><th>".h($o["field"]),"<td><span title='".h($o["collation"])."'>".h($o["full_type"])."</span>",($o["null"]?" <i>NULL</i>":""),($o["auto_increment"]?" <i>".'Auto Increment'."</i>":""),(isset($o["default"])?" <span title='".'VÃ½chozÃ­ hodnota'."'>[<b>".h($o["default"])."</b>]</span>":""),(support("comment")?"<td>".nbsp($o["comment"]):""),"\n";}echo"</table>\n";}function
...
selectColumnsPrint($L,$e){global$id,$od;print_fieldset("select",'Vypsat',$L);$s=0;$L[""]=array();foreach($L
as$y=>$X){$X=$_GET["columns"][$y];$d=select_input(" name='columns[$s][col]'",$e,$X["col"],($y!==""?"selectFieldChange":"selectAddRow"));echo"<div>".($id||$od?"<select name='columns[$s][fun]'>".optionlist(array(-1=>"")+array_filter(array('Funkce'=>$id,'Agregace'=>$od)),$X["fun"])."</select>".on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'",1).script("qsl('select').onchange = function () { helpClose();".($y!==""?"":" qsl('select, input', this.parentNode).onchange();")." };","")."($d)":$d)."</div>\n";$s++;}echo"</div></fieldset>\n";}function
selectSearchPrint($Z,$e,$w){print_fieldset("search",'Vyhledat',$Z);foreach($w
as$s=>$v){if($v["type"]=="FULLTEXT"){echo"<div>(<i>".implode("</i>, <i>",array_map('h',$v["columns"]))."</i>) AGAINST"," <input type='search' name='fulltext[$s]' value='".h($_GET["fulltext"][$s])."'>",script("qsl('input').oninput = selectFieldChange;",""),checkbox("boolean[$s]",1,isset($_GET["boolean"][$s]),"BOOL"),"</div>\n";}}$ab="this.parentNode.firstChild.onchange();";foreach(array_merge((array)$_GET["where"],array(array()))as$s=>$X){if(!$X||("$X[col]$X[val]"!=""&&in_array($X["op"],$this->operators))){echo"<div>".select_input(" name='where[$s][col]'",$e,$X["col"],($X?"selectFieldChange":"selectAddRow"),"(".'kdekoliv'.")"),html_select("where[$s][op]",$this->operators,$X["op"],$ab),"<input type='search' name='where[$s][val]' value='".h($X["val"])."'>",script("mixin(qsl('input'), {oninput: function () { $ab }, onkeydown: selectSearchKeydown, onsearch: selectSearchSearch});",""),"</div>\n";}}echo"</div></fieldset>\n";}function
selectOrderPrint($rf,$e,$w){print_fieldset("sort",'SeÅ™adit',$rf);$s=0;foreach((array)$_GET["order"]as$y=>$X){if($X!=""){echo"<div>".select_input(" name='order[$s]'",$e,$X,"selectFieldChange"),checkbox("desc[$s]",1,isset($_GET["desc"][$y]),'sestupnÄ›')."</div>\n";$s++;}}echo"<div>".select_input(" name='order[$s]'",$e,"","selectAddRow"),checkbox("desc[$s]",1,false,'sestupnÄ›')."</div>\n","</div></fieldset>\n";}function
...
selectLengthPrint($Sh){if($Sh!==null){echo"<fieldset><legend>".'DÃ©lka textÅ¯'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($Sh)."'>","</div></fieldset>\n";}}function
selectActionPrint($w){echo"<fieldset><legend>".'Akce'."</legend><div>","<input type='submit' value='".'Vypsat'."'>"," <span id='noindex' title='".'PrÅ¯chod celÃ© tabulky'."'></span>","<script".nonce().">\n","var indexColumns = ";$e=array();foreach($w
...
messageQuery($G,$Th,$Mc=false){global$x,$m;restart_session();$ud=&get_session("queries");if(!$ud[$_GET["db"]])$ud[$_GET["db"]]=array();if(strlen($G)>1e6)$G=preg_replace('~[\x80-\xFF]+$~','',substr($G,0,1e6))."\n...";$ud[$_GET["db"]][]=array($G,time(),$Th);$qh="sql-".count($ud[$_GET["db"]]);$I="<a href='#$qh' class='toggle'>".'SQL pÅ™Ã­kaz'."</a>\n";if(!$Mc&&($Si=$m->warnings())){$t="warnings-".count($ud[$_GET["db"]]);$I="<a href='#$t' class='toggle'>".'VarovÃ¡nÃ­'."</a>, $I<div id='$t' class='hidden'>\n$Si</div>\n";}return" <span class='time'>".@date("H:i:s")."</span>"." $I<div id='$qh' class='hidden'><pre><code class='jush-$x'>".shorten_utf8($G,1000)."</code></pre>".($Th?" <span class='time'>($Th)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($ud[$_GET["db"]])-1)).'">'.'Upravit'.'</a>':'').'</div>';}function
...
explode("/",$I);}function
editInput($R,$o,$Ja,$Y){if($o["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$Ja value='-1' checked><i>".'pÅ¯vodnÃ­'."</i></label> ":"").($o["null"]?"<label><input type='radio'$Ja value=''".($Y!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$Ja,$o,$Y,0);return"";}function
...
dumpOutput(){$I=array('text'=>'otevÅ™Ã­t','file'=>'uloÅ¾it');if(function_exists('gzencode'))$I['gz']='gzip';return$I;}function
dumpFormat(){return
...
';}$this->databasesPrint($Me);if(DB==""||!$Me){echo"<p class='links'>".(support("sql")?"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])&&!isset($_GET["import"])).">".'SQL pÅ™Ã­kaz'."</a>\n<a href='".h(ME)."import='".bold(isset($_GET["import"])).">".'Import'."</a>\n":"")."";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>\n";}if($_GET["ns"]!==""&&!$Me&&DB!=""){echo'<a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'VytvoÅ™it tabulku'."</a>\n";if(!$T)echo"<p class='message'>".'Å½Ã¡dnÃ© tabulky.'."\n";else$this->tablesPrint($T);}}}function
databasesPrint($Me){global$b,$f;$k=$this->databases();echo'<form action="">
<p id="dbs">
';hidden_fields_get();$Nb=script("mixin(qsl('select'), {onmousedown: dbMouseDown, onchange: dbChange});");echo"<span title='".'databÃ¡ze'."'>".'DB'."</span>: ".($k?"<select name='db'>".optionlist(array(""=>"")+$k,DB)."</select>$Nb":"<input name='db' value='".h(DB)."' autocapitalize='off'>\n"),"<input type='submit' value='".'Vybrat'."'".($k?" class='hidden'":"").">\n";if($Me!="db"&&DB!=""&&$f->select_db(DB)){if(support("scheme")){echo"<br>".'SchÃ©ma'.": <select name='ns'>".optionlist(array(""=>"")+$b->schemas(),$_GET["ns"])."</select>$Nb";if($_GET["ns"]!="")set_schema($_GET["ns"]);}}echo(isset($_GET["sql"])?'<input type="hidden" name="sql" value="">':(isset($_GET["schema"])?'<input type="hidden" name="schema" value="">':(isset($_GET["dump"])?'<input type="hidden" name="dump" value="">':(isset($_GET["privileges"])?'<input type="hidden" name="privileges" value="">':"")))),"</p></form>\n";}function
tablesPrint($T){echo"<ul id='tables'>".script("mixin(qs('#tables'), {onmouseover: menuOver, onmouseout: menuOut});");foreach($T
as$R=>$P){$C=$this->tableName($P);if($C!=""){echo'<li><a href="'.h(ME).'select='.urlencode($R).'"'.bold($_GET["select"]==$R||$_GET["edit"]==$R,"select").">".'vypsat'."</a> ",(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($R).'"'.bold(in_array($R,array($_GET["table"],$_GET["create"],$_GET["indexes"],$_GET["foreign"],$_GET["trigger"])),(is_view($P)?"view":"structure"))." title='".'Zobrazit strukturu'."'>$C</a>":"<span>$C</span>")."\n";}}echo"</ul>\n";}}$b=(function_exists('adminer_object')?adminer_object():new
...
check_invalid_login(){global$b;$Qd=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$Pd=$Qd[$b->bruteForceKey()];$Ve=($Pd[1]>29?$Pd[0]-time():0);if($Ve>0)auth_error(lang(array('PÅ™Ã­liÅ¡ mnoho pokusÅ¯ o pÅ™ihlÃ¡Å¡enÃ­, zkuste to znovu za %d minutu.','PÅ™Ã­liÅ¡ mnoho pokusÅ¯ o pÅ™ihlÃ¡Å¡enÃ­, zkuste to znovu za %d minuty.','PÅ™Ã­liÅ¡ mnoho pokusÅ¯ o pÅ™ihlÃ¡Å¡enÃ­, zkuste to znovu za %d minut.'),ceil($Ve/60)));}$Ka=$_POST["auth"];if($Ka){session_regenerate_id();$Mi=$Ka["driver"];$N=$Ka["server"];$V=$Ka["username"];$F=(string)$Ka["password"];$l=$Ka["db"];set_password($Mi,$N,$V,$F);$_SESSION["db"][$Mi][$N][$V][$l]=true;if($Ka["permanent"]){$y=base64_encode($Mi)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($l);$eg=$b->permanentLogin(true);$Sf[$y]="$y:".base64_encode($eg?encrypt_string($F,$eg):"");cookie("adminer_permanent",implode(" ",$Sf));}if(count($_POST)==1||DRIVER!=$Mi||SERVER!=$N||$_GET["username"]!==$V||DB!=$l)redirect(auth_url($Mi,$N,$V,$l));}elseif($_POST["logout"]){if($sd&&!verify_token()){page_header('OdhlÃ¡sit','NeplatnÃ½ token CSRF. OdeÅ¡lete formulÃ¡Å™ znovu.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$y)set_session($y,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'OdhlÃ¡Å¡enÃ­ probÄ›hlo v poÅ™Ã¡dku.'.' '.sprintf('DÃ­ky za pouÅ¾itÃ­ Admineru, zvaÅ¾te <a href="%s">pÅ™Ã­spÄ›vek</a>.','https://sourceforge.net/donate/index.php?group_id=264133'));}}elseif($Sf&&!$_SESSION["pwds"]){session_regenerate_id();$eg=$b->permanentLogin();foreach($Sf
...
auth_error($n){global$b,$sd;$eh=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$eh]||$_GET[$eh])&&!$sd)$n='Session vyprÅ¡ela, pÅ™ihlaÅ¡te se prosÃ­m znovu.';else{add_invalid_login();$F=get_password();if($F!==null){if($F===false)$n.='<br>'.sprintf('Platnost hlavnÃ­ho hesla vyprÅ¡ela. <a href="https://www.adminer.org/cs/extension/"%s>Implementujte</a> metodu %s, aby platilo stÃ¡le.',target_blank(),'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$eh]&&$_GET[$eh]&&ini_bool("session.use_only_cookies"))$n='Session promÄ›nnÃ© musÃ­ bÃ½t povolenÃ©.';$Ff=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$Ff["lifetime"]);page_header('PÅ™ihlÃ¡sit se',$n,null);echo"<form action='' method='post'>\n","<div>";if(hidden_fields($_POST,array("auth")))echo"<p class='message'>".'Akce bude provedena po ÃºspÄ›Å¡nÃ©m pÅ™ihlÃ¡Å¡enÃ­ se stejnÃ½mi pÅ™ihlaÅ¡ovacÃ­mi Ãºdaji.'."\n";echo"</div>\n";$b->loginForm();echo"</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Å½Ã¡dnÃ© rozÅ¡Ã­Å™enÃ­',sprintf('NenÃ­ dostupnÃ© Å¾Ã¡dnÃ© z podporovanÃ½ch PHP rozÅ¡Ã­Å™enÃ­ (%s).',implode(", ",$Yf)),false);page_footer("auth");exit;}list($xd,$Uf)=explode(":",SERVER,2);if(is_numeric($Uf)&&$Uf<1024)auth_error('PÅ™ipojovÃ¡nÃ­ k privilegovanÃ½m portÅ¯m nenÃ­ povoleno.');check_invalid_login();$f=connect();$m=new
Min_Driver($f);}$re=null;if(!is_object($f)||($re=$b->login($_GET["username"],get_password()))!==true)auth_error((is_string($f)?h($f):(is_string($re)?$re:'NeplatnÃ© pÅ™ihlaÅ¡ovacÃ­ Ãºdaje.')));if($Ka&&$_POST["token"])$_POST["token"]=$di;$n='';if($_POST){if(!verify_token()){$Kd="max_input_vars";$Ce=ini_get($Kd);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$Ce||$X<$Ce)){$Kd=$y;$Ce=$X;}}}$n=(!$_POST["token"]&&$Ce?sprintf('Byl pÅ™ekroÄen maximÃ¡lnÃ­ povolenÃ½ poÄet polÃ­. ZvyÅ¡te prosÃ­m %s.',"'$Kd'"):'NeplatnÃ½ token CSRF. OdeÅ¡lete formulÃ¡Å™ znovu.'.' '.'Pokud jste tento poÅ¾adavek neposlali z Adminera, tak tuto strÃ¡nku zavÅ™ete.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$n=sprintf('PÅ™Ã­liÅ¡ velkÃ¡ POST data. ZmenÅ¡ete data nebo zvyÅ¡te hodnotu konfiguraÄnÃ­ direktivy %s.',"'post_max_size'");if(isset($_GET["sql"]))$n.=' '.'VelkÃ½ SQL soubor mÅ¯Å¾ete nahrÃ¡t pomocÃ­ FTP a importovat ho ze serveru.';}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
...
as$y=>$X){if($X===null)$X="<i>NULL</i>";elseif($Ta[$y]&&!is_utf8($X))$X="<i>".lang(array('%d bajt','%d bajty','%d bajtÅ¯'),strlen($X))."</i>";elseif(!strlen($X))$X="&nbsp;";else{$X=h($X);if($si[$y]==254)$X="<code>$X</code>";}if(isset($qe[$y])&&!$e[$qe[$y]]){if($uf&&$x=="sql"){$R=$J[array_search("table=",$qe)];$_=$qe[$y].urlencode($uf[$R]!=""?$uf[$R]:$R);}else{$_="edit=".urlencode($qe[$y]);foreach($w[$qe[$y]]as$mb=>$Wd)$_.="&where".urlencode("[".bracket_escape($mb)."]")."=".urlencode($J[$Wd]);}$X="<a href='".h(ME.$_)."'>$X</a>";}echo"<td>$X";}}echo($s?"</table>":"<p class='message'>".'Å½Ã¡dnÃ© Å™Ã¡dky.')."\n";return$I;}function
...
edit_type($y,$o,$pb,$cd=array(),$Kc=array()){global$xh,$si,$zi,$hf;$U=$o["type"];echo'<td><select name="',h($y),'[type]" class="type" aria-labelledby="label-type">';if($U&&!isset($si[$U])&&!isset($cd[$U])&&!in_array($U,$Kc))$Kc[]=$U;if($cd)$xh['CizÃ­ klÃ­Äe']=$cd;echo
optionlist(array_merge($Kc,$xh),$U),'</select>
',on_help("getTarget(event).value",1),script("mixin(qsl('select'), {onfocus: function () { lastType = selectValue(this); }, onchange: editingTypeChange});",""),'<td><input name="',h($y),'[length]" value="',h($o["length"]),'" size="3"',(!$o["length"]&&preg_match('~var(char|binary)$~',$U)?" class='required'":""),' aria-labelledby="label-length">',script("mixin(qsl('input'), {onfocus: editingLengthFocus, oninput: editingLengthChange});",""),'<td class="options">';echo"<select name='".h($y)."[collation]'".(preg_match('~(char|text|enum|set)$~',$U)?"":" class='hidden'").'><option value="">('.'porovnÃ¡vÃ¡nÃ­'.')'.optionlist($pb,$o["collation"]).'</select>',($zi?"<select name='".h($y)."[unsigned]'".(!$U||preg_match(number_type(),$U)?"":" class='hidden'").'><option>'.optionlist($zi,$o["unsigned"]).'</select>':''),(isset($o['on_update'])?"<select name='".h($y)."[on_update]'".(preg_match('~timestamp|datetime~',$U)?"":" class='hidden'").'>'.optionlist(array(""=>"(".'PÅ™i zmÄ›nÄ›'.")","CURRENT_TIMESTAMP"),$o["on_update"]).'</select>':''),($cd?"<select name='".h($y)."[on_delete]'".(preg_match("~`~",$U)?"":" class='hidden'")."><option value=''>(".'PÅ™i smazÃ¡nÃ­'.")".optionlist(explode("|",$hf),$o["on_delete"])."</select> ":" ");}function
...
';if($U=="PROCEDURE"){echo'<td>&nbsp;';}echo'<th id="label-name">',($U=="TABLE"?'NÃ¡zev sloupce':'NÃ¡zev parametru'),'<td id="label-type">Typ<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;"></textarea>',script("qs('#enum-edit').onblur = editingLengthBlur;"),'<td id="label-length">DÃ©lka
<td>','Volby';if($U=="TABLE"){echo'<td id="label-null">NULL
<td><input type="radio" name="auto_increment_col" value=""><acronym id="label-ai" title="Auto Increment">AI</acronym>',doc_link(array('sql'=>"example-auto-increment.html",'mariadb'=>"auto_increment/",'sqlite'=>"autoinc.html",'pgsql'=>"datatype.html#DATATYPE-SERIAL",'mssql'=>"ms186775.aspx",)),'<td id="label-default">VÃ½chozÃ­ hodnota
',(support("comment")?"<td id='label-comment'".($vb?"":" class='hidden'").">".'KomentÃ¡Å™':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.6.2")."' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."'>".script("row_count = ".count($p).";"),'</thead>
<tbody>
...
';edit_type("fields[$s]",$o,$pb,$cd);if($U=="TABLE"){echo'<td>',checkbox("fields[$s][null]",1,$o["null"],"","","block","label-null"),'<td><label class="block"><input type="radio" name="auto_increment_col" value="',$s,'"';if($o["auto_increment"]){echo' checked';}echo' aria-labelledby="label-ai"></label><td>',checkbox("fields[$s][has_default]",1,$o["has_default"],"","","","label-default"),'<input name="fields[',$s,'][default]" value="',h($o["default"]),'" aria-labelledby="label-default">',(support("comment")?"<td".($vb?"":" class='hidden'")."><input name='fields[$s][comment]' value='".h($o["comment"])."' maxlength='".(min_version(5.5)?1024:255)."' aria-labelledby='label-comment'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.6.2")."' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."'>&nbsp;"."<input type='image' class='icon' name='up[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=up.gif&version=4.6.2")."' alt='â†‘' title='".'PÅ™esunout nahoru'."'>&nbsp;"."<input type='image' class='icon' name='down[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=down.gif&version=4.6.2")."' alt='â†“' title='".'PÅ™esunout dolÅ¯'."'>&nbsp;":""),($vf==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=4.6.2")."' alt='x' title='".'Odebrat'."'>":"");}}function
...
connect_error(){global$b,$f,$di,$n,$dc;if(DB!=""){header("HTTP/1.1 404 Not Found");page_header('DatabÃ¡ze'.": ".h(DB),'NesprÃ¡vnÃ¡ databÃ¡ze.',true);}else{if($_POST["db"]&&!$n)queries_redirect(substr(ME,0,-1),'DatabÃ¡ze byly odstranÄ›ny.',drop_databases($_POST["db"]));page_header('Vybrat databÃ¡zi',$n,false);echo"<p class='links'>\n";foreach(array('database'=>'VytvoÅ™it databÃ¡zi','privileges'=>'OprÃ¡vnÄ›nÃ­','processlist'=>'Seznam procesÅ¯','variables'=>'PromÄ›nnÃ©','status'=>'Stav',)as$y=>$X){if(support($y))echo"<a href='".h(ME)."$y='>$X</a>\n";}echo"<p>".sprintf('Verze %s: %s pÅ™es PHP rozÅ¡Ã­Å™enÃ­ %s',$dc[DRIVER],"<b>".h($f->server_info)."</b>","<b>$f->extension</b>")."\n","<p>".sprintf('PÅ™ihlÃ¡Å¡en jako: %s',"<b>".h(logged_user())."</b>")."\n";$k=$b->databases();if($k){$Sg=support("scheme");$pb=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' class='checkable'>\n",script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});"),"<thead><tr>".(support("database")?"<td>&nbsp;":"")."<th>".'DatabÃ¡ze'." - <a href='".h(ME)."refresh=1'>".'Obnovit'."</a>"."<td>".'PorovnÃ¡vÃ¡nÃ­'."<td>".'Tabulky'."<td>".'Velikost'." - <a href='".h(ME)."dbsize=1'>".'SpoÄÃ­tat'."</a>".script("qsl('a').onclick = partial(ajaxSetHtml, '".js_escape(ME)."script=connect');","")."</thead>\n";$k=($_GET["dbsize"]?count_tables($k):array_flip($k));foreach($k
as$l=>$T){$Kg=h(ME)."db=".urlencode($l);$t=h("Db-".$l);echo"<tr".odd().">".(support("database")?"<td>".checkbox("db[]",$l,in_array($l,(array)$_POST["db"]),"","","",$t):""),"<th><a href='$Kg' id='$t'>".h($l)."</a>";$ob=nbsp(db_collation($l,$pb));echo"<td>".(support("database")?"<a href='$Kg".($Sg?"&amp;ns=":"")."&amp;database=' title='".'PozmÄ›nit databÃ¡zi'."'>$ob</a>":$ob),"<td align='right'><a href='$Kg&amp;schema=' id='tables-".h($l)."' title='".'SchÃ©ma databÃ¡ze'."'>".($_GET["dbsize"]?$T:"?")."</a>","<td align='right' id='size-".h($l)."'>".($_GET["dbsize"]?db_size($l):"?"),"\n";}echo"</table>\n",(support("database")?"<div class='footer'><div>\n"."<fieldset><legend>".'OznaÄenÃ©'." <span id='selected'></span></legend><div>\n"."<input type='hidden' name='all' value=''>".script("qsl('input').onclick = function () { selectCount('selected', formChecked(this, /^db/)); };")."<input type='submit' name='drop' value='".'Odstranit'."'>".confirm()."\n"."</div></fieldset>\n"."</div></div>\n":""),"<input type='hidden' name='token' value='$di'>\n","</form>\n",script("tableCheck();");}}page_footer("db");}if(isset($_GET["status"]))$_GET["variables"]=$_GET["status"];if(isset($_GET["import"]))$_GET["sql"]=$_GET["import"];if(!(DB!=""?$f->select_db(DB):isset($_GET["sql"])||isset($_GET["dump"])||isset($_GET["database"])||isset($_GET["processlist"])||isset($_GET["privileges"])||isset($_GET["user"])||isset($_GET["variables"])||$_GET["script"]=="connect"||$_GET["script"]=="kill")){if(DB!=""||$_GET["refresh"]){restart_session();set_session("dbs",null);}connect_error();exit;}if(support("scheme")&&DB!=""&&$_GET["ns"]!==""){if(!isset($_GET["ns"]))redirect(preg_replace('~ns=[^&]*&~','',ME)."ns=".get_schema());if(!set_schema($_GET["ns"])){header("HTTP/1.1 404 Not Found");page_header('SchÃ©ma'.": ".h($_GET["ns"]),'NesprÃ¡vnÃ© schÃ©ma.',true);page_footer("ns");exit;}}$hf="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";class
TmpFile{var$handler;var$size;function
__construct(){$this->handler=tmpfile();}function
write($_b){$this->size+=strlen($_b);fwrite($this->handler,$_b);}function
send(){fseek($this->handler,0);fpassthru($this->handler);fclose($this->handler);}}$vc="'(?:''|[^'\\\\]|\\\\.)*'";$Ld="IN|OUT|INOUT";if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["callf"]))$_GET["call"]=$_GET["callf"];if(isset($_GET["function"]))$_GET["procedure"]=$_GET["function"];if(isset($_GET["download"])){$a=$_GET["download"];$p=fields($a);header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));$L=array(idf_escape($_GET["field"]));$H=$m->select($a,$L,array(where($_GET,$p)),$L);$J=($H?$H->fetch_row():array());echo$m->value($J[0],$p[$_GET["field"]]);exit;}elseif(isset($_GET["table"])){$a=$_GET["table"];$p=fields($a);if(!$p)$n=error();$S=table_status1($a,true);$C=$b->tableName($S);page_header(($p&&is_view($S)?$S['Engine']=='materialized view'?'MaterializovanÃ½ pohled':'Pohled':'Tabulka').": ".($C!=""?$C:h($a)),$n);$b->selectLinks($S);$ub=$S["Comment"];if($ub!="")echo"<p class='nowrap'>".'KomentÃ¡Å™'.": ".h($ub)."\n";if($p)$b->tableStructurePrint($p);if(!is_view($S)){if(support("indexes")){echo"<h3 id='indexes'>".'Indexy'."</h3>\n";$w=indexes($a);if($w)$b->tableIndexesPrint($w);echo'<p class="links"><a href="'.h(ME).'indexes='.urlencode($a).'">'.'PozmÄ›nit indexy'."</a>\n";}if(fk_support($S)){echo"<h3 id='foreign-keys'>".'CizÃ­ klÃ­Äe'."</h3>\n";$cd=foreign_keys($a);if($cd){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Zdroj'."<td>".'CÃ­l'."<td>".'PÅ™i smazÃ¡nÃ­'."<td>".'PÅ™i zmÄ›nÄ›'."<td>&nbsp;</thead>\n";foreach($cd
as$C=>$q){echo"<tr title='".h($C)."'>","<th><i>".implode("</i>, <i>",array_map('h',$q["source"]))."</i>","<td><a href='".h($q["db"]!=""?preg_replace('~db=[^&]*~',"db=".urlencode($q["db"]),ME):($q["ns"]!=""?preg_replace('~ns=[^&]*~',"ns=".urlencode($q["ns"]),ME):ME))."table=".urlencode($q["table"])."'>".($q["db"]!=""?"<b>".h($q["db"])."</b>.":"").($q["ns"]!=""?"<b>".h($q["ns"])."</b>.":"").h($q["table"])."</a>","(<i>".implode("</i>, <i>",array_map('h',$q["target"]))."</i>)","<td>".nbsp($q["on_delete"])."\n","<td>".nbsp($q["on_update"])."\n",'<td><a href="'.h(ME.'foreign='.urlencode($a).'&name='.urlencode($C)).'">'.'ZmÄ›nit'.'</a>';}echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'foreign='.urlencode($a).'">'.'PÅ™idat cizÃ­ klÃ­Ä'."</a>\n";}}if(support(is_view($S)?"view_trigger":"trigger")){echo"<h3 id='triggers'>".'Triggery'."</h3>\n";$pi=triggers($a);if($pi){echo"<table cellspacing='0'>\n";foreach($pi
as$y=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($y)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($y))."'>".'ZmÄ›nit'."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.'PÅ™idat trigger'."</a>\n";}}elseif(isset($_GET["schema"])){page_header('SchÃ©ma databÃ¡ze',"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Gh=array();$Hh=array();$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE["adminer_schema-".str_replace(".","_",DB)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$xe,PREG_SET_ORDER);foreach($xe
...
<table cellspacing="0">
';$Ob=array('','USE','DROP+CREATE','CREATE');$Ih=array('','DROP+CREATE','CREATE');$Lb=array('','TRUNCATE+INSERT','INSERT');if($x=="sql")$Lb[]='INSERT+UPDATE';parse_str($_COOKIE["adminer_export"],$J);if(!$J)$J=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");if(!isset($J["events"])){$J["routines"]=$J["events"]=($_GET["dump"]=="");$J["triggers"]=$J["table_style"];}echo"<tr><th>".'VÃ½stup'."<td>".html_select("output",$b->dumpOutput(),$J["output"],0)."\n";echo"<tr><th>".'FormÃ¡t'."<td>".html_select("format",$b->dumpFormat(),$J["format"],0)."\n";echo($x=="sqlite"?"":"<tr><th>".'DatabÃ¡ze'."<td>".html_select('db_style',$Ob,$J["db_style"]).(support("routine")?checkbox("routines",1,$J["routines"],'Procedury a funkce'):"").(support("event")?checkbox("events",1,$J["events"],'UdÃ¡losti'):"")),"<tr><th>".'Tabulky'."<td>".html_select('table_style',$Ih,$J["table_style"]).checkbox("auto_increment",1,$J["auto_increment"],'Auto Increment').(support("trigger")?checkbox("triggers",1,$J["triggers"],'Triggery'):""),"<tr><th>".'Data'."<td>".html_select('data_style',$Lb,$J["data_style"]),'</table>
<p><input type="submit" value="Export">
<input type="hidden" name="token" value="',$di,'">

<table cellspacing="0">
',script("qsl('table').onclick = dumpClick;");$ag=array();if(DB!=""){$eb=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$eb>".'Tabulky'."</label>".script("qs('#check-tables').onclick = partial(formCheck, /^tables\\[/);",""),"<th style='text-align: right;'><label class='block'>".'Data'."<input type='checkbox' id='check-data'$eb></label>".script("qs('#check-data').onclick = partial(formCheck, /^data\\[/);",""),"</thead>\n";$Pi="";$Jh=tables_list();foreach($Jh
...
script("ajaxSetHtml('".js_escape(ME)."script=db');");}else{echo"<thead><tr><th style='text-align: left;'>","<label class='block'><input type='checkbox' id='check-databases'".($a==""?" checked":"").">".'DatabÃ¡ze'."</label>",script("qs('#check-databases').onclick = partial(formCheck, /^databases\\[/);",""),"</thead>\n";$k=$b->databases();if($k){foreach($k
as$l){if(!information_schema($l)){$Zf=preg_replace('~_.*~','',$l);echo"<tr><td>".checkbox("databases[]",$l,$a==""||$a=="$Zf%",$l,"","block")."\n";$ag[$Zf]++;}}}else
echo"<tr><td><textarea name='databases' rows='10' cols='20'></textarea>";}echo'</table>
</form>
';$Vc=true;foreach($ag
as$y=>$X){if($y!=""&&$X>1){echo($Vc?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$y%")."'>".h($y)."</a>";$Vc=false;}}}elseif(isset($_GET["privileges"])){page_header('OprÃ¡vnÄ›nÃ­');echo'<p class="links"><a href="'.h(ME).'user=">'.'VytvoÅ™it uÅ¾ivatele'."</a>";$H=$f->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q(DB)." LIKE Db")." ORDER BY Host, User");$jd=$H;if(!$H)$H=$f->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");echo"<form action=''><p>\n";hidden_fields_get();echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($jd?"":"<input type='hidden' name='grant' value=''>\n"),"<table cellspacing='0'>\n","<thead><tr><th>".'UÅ¾ivatel'."<th>".'Server'."<th>&nbsp;</thead>\n";while($J=$H->fetch_assoc())echo'<tr'.odd().'><td>'.h($J["User"])."<td>".h($J["Host"]).'<td><a href="'.h(ME.'user='.urlencode($J["User"]).'&host='.urlencode($J["Host"])).'">'.'Upravit'."</a>\n";if(!$jd||DB!="")echo"<tr".odd()."><td><input name='user' autocapitalize='off'><td><input name='host' value='localhost' autocapitalize='off'><td><input type='submit' value='".'Upravit'."'>\n";echo"</table>\n","</form>\n";}elseif(isset($_GET["sql"])){if(!$n&&$_POST["export"]){dump_headers("sql");$b->dumpTable("","");$b->dumpData("","table",$_POST["query"]);exit;}restart_session();$vd=&get_session("queries");$ud=&$vd[DB];if(!$n&&$_POST["clear"]){$ud=array();redirect(remove_from_uri("history"));}page_header((isset($_GET["import"])?'Import':'SQL pÅ™Ã­kaz'),$n);if(!$n&&$_POST){$gd=false;if(!isset($_GET["import"]))$G=$_POST["query"];elseif($_POST["webfile"]){$ph=$b->importServerPath();$gd=@fopen((file_exists($ph)?$ph:"compress.zlib://$ph.gz"),"rb");$G=($gd?fread($gd,1e6):false);}else$G=get_file("sql_file",true);if(is_string($G)){if(function_exists('memory_get_usage'))@ini_set("memory_limit",max(ini_bytes("memory_limit"),2*strlen($G)+memory_get_usage()+8e6));if($G!=""&&strlen($G)<1e6){$lg=$G.(preg_match("~;[ \t\r\n]*\$~",$G)?"":";");if(!$ud||reset(end($ud))!=$lg){restart_session();$ud[]=array($lg,time());set_session("queries",$vd);stop_session();}}$mh="(?:\\s|/\\*[\s\S]*?\\*/|(?:#|-- )[^\n]*\n?|--\r?\n)";$Ub=";";$D=0;$sc=true;$g=connect();if(is_object($g)&&DB!="")$g->select_db(DB);$tb=0;$xc=array();$Hf='[\'"'.($x=="sql"?'`#':($x=="sqlite"?'`[':($x=="mssql"?'[':''))).']|/\\*|-- |$'.($x=="pgsql"?'|\\$[^$]*\\$':'');$fi=microtime(true);parse_str($_COOKIE["adminer_export"],$xa);$jc=$b->dumpFormat();unset($jc["sql"]);while($G!=""){if(!$D&&preg_match("~^$mh*+DELIMITER\\s+(\\S+)~i",$G,$B)){$Ub=$B[1];$G=substr($G,strlen($B[0]));}else{preg_match('('.preg_quote($Ub)."\\s*|$Hf)",$G,$B,PREG_OFFSET_CAPTURE,$D);list($ed,$Vf)=$B[0];if(!$ed&&$gd&&!feof($gd))$G.=fread($gd,1e5);else{if(!$ed&&rtrim($G)=="")break;$D=$Vf+strlen($ed);if($ed&&rtrim($ed)!=$Ub){while(preg_match('('.($ed=='/*'?'\\*/':($ed=='['?']':(preg_match('~^-- |^#~',$ed)?"\n":preg_quote($ed)."|\\\\."))).'|$)s',$G,$B,PREG_OFFSET_CAPTURE,$D)){$Pg=$B[0][0];if(!$Pg&&$gd&&!feof($gd))$G.=fread($gd,1e5);else{$D=$B[0][1]+strlen($Pg);if($Pg[0]!="\\")break;}}}else{$sc=false;$lg=substr($G,0,$Vf);$tb++;$dg="<pre id='sql-$tb'><code class='jush-$x'>".$b->sqlCommandQuery($lg)."</code></pre>\n";if($x=="sqlite"&&preg_match("~^$mh*+ATTACH\\b~i",$lg,$B)){echo$dg,"<p class='error'>".'Dotazy ATTACH nejsou podporovanÃ©.'."\n";$xc[]=" <a href='#sql-$tb'>$tb</a>";if($_POST["error_stops"])break;}else{if(!$_POST["only_errors"]){echo$dg;ob_flush();flush();}$th=microtime(true);if($f->multi_query($lg)&&is_object($g)&&preg_match("~^$mh*+USE\\b~i",$lg))$g->query($lg);do{$H=$f->store_result();if($f->error){echo($_POST["only_errors"]?$dg:""),"<p class='error'>".'Chyba v dotazu'.($f->errno?" ($f->errno)":"").": ".error()."\n";$xc[]=" <a href='#sql-$tb'>$tb</a>";if($_POST["error_stops"])break
2;}else{$Th=" <span class='time'>(".format_time($th).")</span>".(strlen($lg)<1000?" <a href='".h(ME)."sql=".urlencode(trim($lg))."'>".'Upravit'."</a>":"");$za=$f->affected_rows;$Si=($_POST["only_errors"]?"":$m->warnings());$Ti="warnings-$tb";if($Si)$Th.=", <a href='#$Ti'>".'VarovÃ¡nÃ­'."</a>".script("qsl('a').onclick = partial(toggle, '$Ti');","");$Ec=null;$Fc="explain-$tb";if(is_object($H)){$z=$_POST["limit"];$uf=select($H,$g,array(),$z);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n";$Xe=$H->num_rows;echo"<p>".($Xe?($z&&$Xe>$z?sprintf('%d / ',$z):"").lang(array('%d Å™Ã¡dek','%d Å™Ã¡dky','%d Å™Ã¡dkÅ¯'),$Xe):""),$Th;if($g&&preg_match("~^($mh|\\()*+SELECT\\b~i",$lg)&&($Ec=explain($g,$lg)))echo", <a href='#$Fc'>Explain</a>".script("qsl('a').onclick = partial(toggle, '$Fc');","");$t="export-$tb";echo", <a href='#$t'>".'Export'."</a>".script("qsl('a').onclick = partial(toggle, '$t');","")."<span id='$t' class='hidden'>: ".html_select("output",$b->dumpOutput(),$xa["output"])." ".html_select("format",$jc,$xa["format"])."<input type='hidden' name='query' value='".h($lg)."'>"." <input type='submit' name='export' value='".'Export'."'><input type='hidden' name='token' value='$di'></span>\n"."</form>\n";}}else{if(preg_match("~^$mh*+(CREATE|DROP|ALTER)$mh++(DATABASE|SCHEMA)\\b~i",$lg)){restart_session();set_session("dbs",null);stop_session();}if(!$_POST["only_errors"])echo"<p class='message' title='".h($f->info)."'>".lang(array('PÅ™Ã­kaz probÄ›hl v poÅ™Ã¡dku, byl zmÄ›nÄ›n %d zÃ¡znam.','PÅ™Ã­kaz probÄ›hl v poÅ™Ã¡dku, byly zmÄ›nÄ›ny %d zÃ¡znamy.','PÅ™Ã­kaz probÄ›hl v poÅ™Ã¡dku, bylo zmÄ›nÄ›no %d zÃ¡znamÅ¯.'),$za)."$Th\n";}echo($Si?"<div id='$Ti' class='hidden'>\n$Si</div>\n":"");if($Ec){echo"<div id='$Fc' class='hidden'>\n";select($Ec,$g,$uf);echo"</div>\n";}}$th=microtime(true);}while($f->next_result());}$G=substr($G,$D);$D=0;}}}}if($sc)echo"<p class='message'>".'Å½Ã¡dnÃ© pÅ™Ã­kazy k vykonÃ¡nÃ­.'."\n";elseif($_POST["only_errors"]){echo"<p class='message'>".lang(array('%d pÅ™Ã­kaz probÄ›hl v poÅ™Ã¡dku.','%d pÅ™Ã­kazy probÄ›hly v poÅ™Ã¡dku.','%d pÅ™Ã­kazÅ¯ probÄ›hlo v poÅ™Ã¡dku.'),$tb-count($xc))," <span class='time'>(".format_time($fi).")</span>\n";}elseif($xc&&$tb>1)echo"<p class='error'>".'Chyba v dotazu'.": ".implode("",$xc)."\n";}else
echo"<p class='error'>".upload_error($G)."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';$Bc="<input type='submit' value='".'ProvÃ©st'."' title='Ctrl+Enter'>";if(!isset($_GET["import"])){$lg=$_GET["sql"];if($_POST)$lg=$_POST["query"];elseif($_GET["history"]=="all")$lg=$ud;elseif($_GET["history"]!="")$lg=$ud[$_GET["history"]][0];echo"<p>";textarea("query",$lg,20);echo($_POST?"":script("qs('textarea').focus();")),"<p>$Bc\n",'Limit Å™Ã¡dek'.": <input type='number' name='limit' class='size' value='".h($_POST?$_POST["limit"]:$_GET["limit"])."'>\n";}else{echo"<fieldset><legend>".'NahrÃ¡nÃ­ souboru'."</legend><div>",(ini_bool("file_uploads")?"SQL (&lt; ".ini_get("upload_max_filesize")."B): <input type='file' name='sql_file[]' multiple>\n$Bc":'NahrÃ¡vÃ¡nÃ­ souborÅ¯ nenÃ­ povoleno.'),"</div></fieldset>\n","<fieldset><legend>".'Ze serveru'."</legend><div>",sprintf('Soubor %s na webovÃ©m serveru',"<code>".h($b->importServerPath()).(extension_loaded("zlib")?"[.gz]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustit soubor'.'">',"</div></fieldset>\n","<p>";}echo
checkbox("error_stops",1,($_POST?$_POST["error_stops"]:isset($_GET["import"])),'Zastavit pÅ™i chybÄ›')."\n",checkbox("only_errors",1,($_POST?$_POST["only_errors"]:isset($_GET["import"])),'Zobrazit pouze chyby')."\n","<input type='hidden' name='token' value='$di'>\n";if(!isset($_GET["import"])&&$ud){print_fieldset("history",'Historie',$_GET["history"]!="");for($X=end($ud);$X;$X=prev($ud)){$y=key($ud);list($lg,$Th,$nc)=$X;echo'<a href="'.h(ME."sql=&history=$y").'">'.'Upravit'."</a>"." <span class='time' title='".@date('Y-m-d',$Th)."'>".@date("H:i:s",$Th)."</span>"." <code class='jush-$x'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$lg)))),80,"</code>").($nc?" <span class='time'>($nc)</span>":"")."<br>\n";}echo"<input type='submit' name='clear' value='".'VyÄistit'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Upravit vÅ¡e'."</a>\n","</div></fieldset>\n";}echo'</form>
...
as$C=>$o){if(!isset($o["privileges"][$_i?"update":"insert"])||$b->fieldName($o)=="")unset($p[$C]);}if($_POST&&!$n&&!isset($_GET["select"])){$A=$_POST["referer"];if($_POST["insert"])$A=($_i?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$A))$A=ME."select=".urlencode($a);$w=indexes($a);$vi=unique_array($_GET["where"],$w);$og="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($A,'PoloÅ¾ka byla smazÃ¡na.',$m->delete($a,$og,!$vi));else{$O=array();foreach($p
as$C=>$o){$X=process_input($o);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($_i){if(!$O)redirect($A);queries_redirect($A,'PoloÅ¾ka byla aktualizovÃ¡na.',$m->update($a,$O,$og,!$vi));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$H=$m->insert($a,$O);$ie=($H?last_id():0);queries_redirect($A,sprintf('PoloÅ¾ka%s byla vloÅ¾ena.',($ie?" $ie":"")),$H);}}}$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($Z){$L=array();foreach($p
...
<p>
';if(support("columns")||$a==""){echo'NÃ¡zev tabulky: <input name="name" maxlength="64" value="',h($J["name"]),'" autocapitalize="off">
';if($a==""&&!$_POST)echo
script("focus(qs('#form')['name']);");echo($uc?"<select name='Engine'>".optionlist(array(""=>"(".'ÃºloÅ¾iÅ¡tÄ›'.")")+$uc,$J["Engine"])."</select>".on_help("getTarget(event).value",1).script("qsl('select').onchange = helpClose;"):""),' ',($pb&&!preg_match("~sqlite|mssql~",$x)?html_select("Collation",array(""=>"(".'porovnÃ¡vÃ¡nÃ­'.")")+$pb,$J["Collation"]):""),' <input type="submit" value="UloÅ¾it">
';}echo'
...
',checkbox("defaults",1,!$_POST||$_POST["defaults"],'VÃ½chozÃ­ hodnoty',"columnShow(this.checked, 5)","jsonly"),($_POST?"":script("editingHideDefaults();")),(support("comment")?"<label><input type='checkbox' name='comments' value='1' class='jsonly'".($vb?" checked":"").">".'KomentÃ¡Å™'."</label>".script("qsl('input').onclick = partial(editingCommentsClick, true);").' <input name="Comment" value="'.h($J["Comment"]).'" maxlength="'.(min_version(5.5)?2048:60).'"'.($vb?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="UloÅ¾it">
';}echo'
';if($a!=""){echo'<input type="submit" name="drop" value="Odstranit">',confirm(sprintf('Odstranit %s?',$a));}if(support("partitioning")){$Kf=preg_match('~RANGE|LIST~',$J["partition_by"]);print_fieldset("partition",'RozdÄ›lit podle',$J["partition_by"]);echo'<p>
...
OddÃ­ly: <input type="number" name="partitions" class="size',($Kf||!$J["partition_by"]?" hidden":""),'" value="',h($J["partitions"]),'">
...
<th><input type="submit" class="wayoff">Sloupec (dÃ©lka)
<th id="label-name">NÃ¡zev
<th><noscript><input type='image' class='icon' name='add[0]' src='" . h(preg_replace("~\\?.*~", "", ME) . "?file=plus.gif&version=4.6.2") . "' alt='+' title='PÅ™idat dalÅ¡Ã­'></noscript>&nbsp;
</thead>
<?php
if($bg){echo"<tr><td>PRIMARY<td>";foreach($bg["columns"]as$y=>$d){echo
select_input(" disabled",$p,$d),"<label><input disabled type='checkbox'>".'sestupnÄ›'."</label> ";}echo"<td><td>\n";}$Wd=1;foreach($J["indexes"]as$v){if(!$_POST["drop_col"]||$Wd!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$Wd][type]",array(-1=>"")+$Dd,$v["type"],($Wd==count($J["indexes"])?"indexesAddRow.call(this);":1),"label-type"),"<td>";ksort($v["columns"]);$s=1;foreach($v["columns"]as$y=>$d){echo"<span>".select_input(" name='indexes[$Wd][columns][$s]' title='".'Sloupec'."'",($p?array_combine($p,$p):$p),$d,"partial(".($s==count($v["columns"])?"indexesAddColumn":"indexesChangeColumn").", '".js_escape($x=="sql"?"":$_GET["indexes"]."_")."')"),($x=="sql"||$x=="mssql"?"<input type='number' name='indexes[$Wd][lengths][$s]' class='size' value='".h($v["lengths"][$y])."' title='".'DÃ©lka'."'>":""),($x!="sql"?checkbox("indexes[$Wd][descs][$s]",1,$v["descs"][$y],'sestupnÄ›'):"")," </span>";$s++;}echo"<td><input name='indexes[$Wd][name]' value='".h($v["name"])."' autocapitalize='off' aria-labelledby='label-name'>\n","<td><input type='image' class='icon' name='drop_col[$Wd]' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=4.6.2")."' alt='x' title='".'Odebrat'."'>".script("qsl('input').onclick = partial(editingRemoveRow, 'indexes\$1[type]');");}$Wd++;}echo'</table>
<p>
<input type="submit" value="UloÅ¾it">
<input type="hidden" name="token" value="',$di,'">
</form>
';}elseif(isset($_GET["database"])){$J=$_POST;if($_POST&&!$n&&!isset($_POST["add_x"])){$C=trim($J["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'DatabÃ¡ze byla odstranÄ›na.',drop_databases(array(DB)));}elseif(DB!==$C){if(DB!=""){$_GET["db"]=$C;queries_redirect(preg_replace('~\bdb=[^&]*&~','',ME)."db=".urlencode($C),'DatabÃ¡ze byla pÅ™ejmenovÃ¡na.',rename_database($C,$J["collation"]));}else{$k=explode("\n",str_replace("\r","",$C));$zh=true;$he="";foreach($k
as$l){if(count($k)==1||$l!=""){if(!create_database($l,$J["collation"]))$zh=false;$he=$l;}}restart_session();set_session("dbs",null);queries_redirect(ME."db=".urlencode($he),'DatabÃ¡ze byla vytvoÅ™ena.',$zh);}}else{if(!$J["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$J["collation"])?" COLLATE $J[collation]":""),substr(ME,0,-1),'DatabÃ¡ze byla zmÄ›nÄ›na.');}}page_header(DB!=""?'PozmÄ›nit databÃ¡zi':'VytvoÅ™it databÃ¡zi',$n,array(),h(DB));$pb=collations();$C=DB;if($_POST)$C=$J["name"];elseif(DB!="")$J["collation"]=db_collation(DB,$pb);elseif($x=="sql"){foreach(get_vals("SHOW GRANTS")as$jd){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$jd,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($C,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>':'<input name="name" id="name" value="'.h($C).'" maxlength="64" autocapitalize="off">')."\n".($pb?html_select("collation",array(""=>"(".'porovnÃ¡vÃ¡nÃ­'.")")+$pb,$J["collation"]).doc_link(array('sql'=>"charset-charsets.html",'mariadb'=>"supported-character-sets-and-collations/",'mssql'=>"ms187963.aspx",)):""),script("focus(qs('#name'));"),'<input type="submit" value="UloÅ¾it">
';if(DB!="")echo"<input type='submit' name='drop' value='".'Odstranit'."'>".confirm(sprintf('Odstranit %s?',DB))."\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.6.2")."' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."'>\n";echo'<input type="hidden" name="token" value="',$di,'">
</form>
';}elseif(isset($_GET["scheme"])){$J=$_POST;if($_POST&&!$n){$_=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$_,'SchÃ©ma bylo odstranÄ›no.');else{$C=trim($J["name"]);$_.=urlencode($C);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($C),$_,'SchÃ©ma bylo vytvoÅ™eno.');elseif($_GET["ns"]!=$C)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($C),$_,'SchÃ©ma bylo zmÄ›nÄ›no.');else
redirect($_);}}page_header($_GET["ns"]!=""?'PozmÄ›nit schÃ©ma':'VytvoÅ™it schÃ©ma',$n);if(!$J)$J["name"]=$_GET["ns"];echo'
<form action="" method="post">
<p><input name="name" id="name" value="',h($J["name"]),'" autocapitalize="off">
',script("focus(qs('#name'));"),'<input type="submit" value="UloÅ¾it">
';if($_GET["ns"]!="")echo"<input type='submit' name='drop' value='".'Odstranit'."'>".confirm(sprintf('Odstranit %s?',$_GET["ns"]))."\n";echo'<input type="hidden" name="token" value="',$di,'">
</form>
';}elseif(isset($_GET["call"])){$da=($_GET["name"]?$_GET["name"]:$_GET["call"]);page_header('Zavolat'.": ".h($da),$n);$Lg=routine($_GET["call"],(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$Bd=array();$Af=array();foreach($Lg["fields"]as$s=>$o){if(substr($o["inout"],-3)=="OUT")$Af[$s]="@".idf_escape($o["field"])." AS ".idf_escape($o["field"]);if(!$o["inout"]||substr($o["inout"],0,2)=="IN")$Bd[]=$s;}if(!$n&&$_POST){$Za=array();foreach($Lg["fields"]as$y=>$o){if(in_array($y,$Bd)){$X=process_input($o);if($X===false)$X="''";if(isset($Af[$y]))$f->query("SET @".idf_escape($o["field"])." = $X");}$Za[]=(isset($Af[$y])?"@".idf_escape($o["field"]):$X);}$G=(isset($_GET["callf"])?"SELECT":"CALL")." ".table($da)."(".implode(", ",$Za).")";$th=microtime(true);$H=$f->multi_query($G);$za=$f->affected_rows;echo$b->selectQuery($G,$th,!$H);if(!$H)echo"<p class='error'>".error()."\n";else{$g=connect();if(is_object($g))$g->select_db(DB);do{$H=$f->store_result();if(is_object($H))select($H,$g);else
echo"<p class='message'>".lang(array('Procedura byla zavolÃ¡na, byl zmÄ›nÄ›n %d zÃ¡znam.','Procedura byla zavolÃ¡na, byly zmÄ›nÄ›ny %d zÃ¡znamy.','Procedura byla zavolÃ¡na, bylo zmÄ›nÄ›no %d zÃ¡znamÅ¯.'),$za)."\n";}while($f->next_result());if($Af)select($f->query("SELECT ".implode(", ",$Af)));}}echo'
<form action="" method="post">
...
<table cellspacing="0">
<thead><tr><th id="label-source">Zdroj<th id="label-target">CÃ­l</thead>
...
<p>
PÅ™i smazÃ¡nÃ­: ',html_select("on_delete",array(-1=>"")+explode("|",$hf),$J["on_delete"]),' PÅ™i zmÄ›nÄ›: ',html_select("on_update",array(-1=>"")+explode("|",$hf),$J["on_update"]),doc_link(array('sql'=>"innodb-foreign-key-constraints.html",'mariadb'=>"foreign-keys/",'pgsql'=>"sql-createtable.html#SQL-CREATETABLE-REFERENCES",'mssql'=>"ms174979.aspx",'oracle'=>"clauses002.htm#sthref2903",)),'<p>
<input type="submit" value="UloÅ¾it">
<noscript><p><input type="submit" name="add" value="PÅ™idat sloupec"></noscript>
';}if($C!=""){echo'<input type="submit" name="drop" value="Odstranit">',confirm(sprintf('Odstranit %s?',$C));}echo'<input type="hidden" name="token" value="',$di,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$J=$_POST;$yf="VIEW";if($x=="pgsql"&&$a!=""){$P=table_status($a);$yf=strtoupper($P["Engine"]);}if($_POST&&!$n){$C=trim($J["name"]);$Ga=" AS\n$J[select]";$A=ME."table=".urlencode($C);$Ee='Pohled byl zmÄ›nÄ›n.';$U=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$C&&$x!="sqlite"&&$U=="VIEW"&&$yf=="VIEW")query_redirect(($x=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ga,$A,$Ee);else{$Oh=$C."_adminer_".uniqid();drop_create("DROP $yf ".table($a),"CREATE $U ".table($C).$Ga,"DROP $U ".table($C),"CREATE $U ".table($Oh).$Ga,"DROP $U ".table($Oh),($_POST["drop"]?substr(ME,0,-1):$A),'Pohled byl odstranÄ›n.',$Ee,'Pohled byl vytvoÅ™en.',$a,$C);}}if(!$_POST&&$a!=""){$J=view($a);$J["name"]=$a;$J["materialized"]=($yf!="VIEW");if(!$n)$n=error();}page_header(($a!=""?'PozmÄ›nit pohled':'VytvoÅ™it pohled'),$n,array("table"=>$a),h($a));echo'
<form action="" method="post">
<p>NÃ¡zev: <input name="name" value="',h($J["name"]),'" maxlength="64" autocapitalize="off">
',(support("materializedview")?" ".checkbox("materialized",1,$J["materialized"],'MaterializovanÃ½ pohled'):""),'<p>';textarea("select",$J["select"]);echo'<p>
<input type="submit" value="UloÅ¾it">
';if($a!=""){echo'<input type="submit" name="drop" value="Odstranit">',confirm(sprintf('Odstranit %s?',$a));}echo'<input type="hidden" name="token" value="',$di,'">
</form>
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$Od=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$vh=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");$J=$_POST;if($_POST&&!$n){if($_POST["drop"])query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'UdÃ¡lost byla odstranÄ›na.');elseif(in_array($J["INTERVAL_FIELD"],$Od)&&isset($vh[$J["STATUS"]])){$Qg="\nON SCHEDULE ".($J["INTERVAL_VALUE"]?"EVERY ".q($J["INTERVAL_VALUE"])." $J[INTERVAL_FIELD]".($J["STARTS"]?" STARTS ".q($J["STARTS"]):"").($J["ENDS"]?" ENDS ".q($J["ENDS"]):""):"AT ".q($J["STARTS"]))." ON COMPLETION".($J["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'UdÃ¡lost byla zmÄ›nÄ›na.':'UdÃ¡lost byla vytvoÅ™ena.'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$Qg.($aa!=$J["EVENT_NAME"]?"\nRENAME TO ".idf_escape($J["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($J["EVENT_NAME"]).$Qg)."\n".$vh[$J["STATUS"]]." COMMENT ".q($J["EVENT_COMMENT"]).rtrim(" DO\n$J[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'PozmÄ›nit udÃ¡lost'.": ".h($aa):'VytvoÅ™it udÃ¡lost'),$n);if(!$J&&$aa!=""){$K=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$J=reset($K);}echo'
<form action="" method="post">
<table cellspacing="0">
<tr><th>NÃ¡zev<td><input name="EVENT_NAME" value="',h($J["EVENT_NAME"]),'" maxlength="64" autocapitalize="off">
<tr><th title="datetime">ZaÄÃ¡tek<td><input name="STARTS" value="',h("$J[EXECUTE_AT]$J[STARTS]"),'">
<tr><th title="datetime">Konec<td><input name="ENDS" value="',h($J["ENDS"]),'">
<tr><th>KaÅ¾dÃ½ch<td><input type="number" name="INTERVAL_VALUE" value="',h($J["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Od,$J["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$vh,$J["STATUS"]),'<tr><th>KomentÃ¡Å™<td><input name="EVENT_COMMENT" value="',h($J["EVENT_COMMENT"]),'" maxlength="64">
<tr><th>&nbsp;<td>',checkbox("ON_COMPLETION","PRESERVE",$J["ON_COMPLETION"]=="PRESERVE",'Po dokonÄenÃ­ zachovat'),'</table>
<p>';textarea("EVENT_DEFINITION",$J["EVENT_DEFINITION"]);echo'<p>
<input type="submit" value="UloÅ¾it">
';if($aa!=""){echo'<input type="submit" name="drop" value="Odstranit">',confirm(sprintf('Odstranit %s?',$aa));}echo'<input type="hidden" name="token" value="',$di,'">
</form>
';}elseif(isset($_GET["procedure"])){$da=($_GET["name"]?$_GET["name"]:$_GET["procedure"]);$Lg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$J=$_POST;$J["fields"]=(array)$J["fields"];if($_POST&&!process_fields($J["fields"])&&!$n){$vf=routine($_GET["procedure"],$Lg);$Oh="$J[name]_adminer_".uniqid();drop_create("DROP $Lg ".routine_id($da,$vf),create_routine($Lg,$J),"DROP $Lg ".routine_id($J["name"],$J),create_routine($Lg,array("name"=>$Oh)+$J),"DROP $Lg ".routine_id($Oh,$J),substr(ME,0,-1),'Procedura byla odstranÄ›na.','Procedura byla zmÄ›nÄ›na.','Procedura byla vytvoÅ™ena.',$da,$J["name"]);}page_header(($da!=""?(isset($_GET["function"])?'ZmÄ›nit funkci':'ZmÄ›nit proceduru').": ".h($da):(isset($_GET["function"])?'VytvoÅ™it funkci':'VytvoÅ™it proceduru')),$n);if(!$_POST&&$da!=""){$J=routine($_GET["procedure"],$Lg);$J["name"]=$da;}$pb=get_vals("SHOW CHARACTER SET");sort($pb);$Mg=routine_languages();echo'
<form action="" method="post" id="form">
<p>NÃ¡zev: <input name="name" value="',h($J["name"]),'" maxlength="64" autocapitalize="off">
',($Mg?'Jazyk'.": ".html_select("language",$Mg,$J["language"])."\n":""),'<input type="submit" value="UloÅ¾it">
<table cellspacing="0" class="nowrap">
';edit_fields($J["fields"],$pb,$Lg);if(isset($_GET["function"])){echo"<tr><td>".'NÃ¡vratovÃ½ typ';edit_type("returns",$J["returns"],$pb,array(),($x=="pgsql"?array("void","trigger"):array()));}echo'</table>
<p>';textarea("definition",$J["definition"]);echo'<p>
<input type="submit" value="UloÅ¾it">
';if($da!=""){echo'<input type="submit" name="drop" value="Odstranit">',confirm(sprintf('Odstranit %s?',$da));}echo'<input type="hidden" name="token" value="',$di,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];$J=$_POST;if($_POST&&!$n){$_=substr(ME,0,-1);$C=trim($J["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$_,'Sekvence byla odstranÄ›na.');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($C),$_,'Sekvence byla vytvoÅ™ena.');elseif($fa!=$C)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($C),$_,'Sekvence byla zmÄ›nÄ›na.');else
redirect($_);}page_header($fa!=""?'PozmÄ›nit sekvenci'.": ".h($fa):'VytvoÅ™it sekvenci',$n);if(!$J)$J["name"]=$fa;echo'
<form action="" method="post">
<p><input name="name" value="',h($J["name"]),'" autocapitalize="off">
<input type="submit" value="UloÅ¾it">
';if($fa!="")echo"<input type='submit' name='drop' value='".'Odstranit'."'>".confirm(sprintf('Odstranit %s?',$fa))."\n";echo'<input type="hidden" name="token" value="',$di,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];$J=$_POST;if($_POST&&!$n){$_=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$_,'Typ byl odstranÄ›n.');else
query_redirect("CREATE TYPE ".idf_escape(trim($J["name"]))." $J[as]",$_,'Typ byl vytvoÅ™en.');}page_header($ga!=""?'PozmÄ›nit typ'.": ".h($ga):'VytvoÅ™it typ',$n);if(!$J)$J["as"]="AS ";echo'
<form action="" method="post">
<p>
';if($ga!="")echo"<input type='submit' name='drop' value='".'Odstranit'."'>".confirm(sprintf('Odstranit %s?',$ga))."\n";else{echo"<input name='name' value='".h($J['name'])."' autocapitalize='off'>\n";textarea("as",$J["as"]);echo"<p><input type='submit' value='".'UloÅ¾it'."'>\n";}echo'<input type="hidden" name="token" value="',$di,'">
</form>
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$C=$_GET["name"];$oi=trigger_options();$J=(array)trigger($C)+array("Trigger"=>$a."_bi");if($_POST){if(!$n&&in_array($_POST["Timing"],$oi["Timing"])&&in_array($_POST["Event"],$oi["Event"])&&in_array($_POST["Type"],$oi["Type"])){$gf=" ON ".table($a);$ec="DROP TRIGGER ".idf_escape($C).($x=="pgsql"?$gf:"");$A=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($ec,$A,'Trigger byl odstranÄ›n.');else{if($C!="")queries($ec);queries_redirect($A,($C!=""?'Trigger byl zmÄ›nÄ›n.':'Trigger byl vytvoÅ™en.'),queries(create_trigger($gf,$_POST)));if($C!="")queries(create_trigger($gf,$J+array("Type"=>reset($oi["Type"]))));}}$J=$_POST;}page_header(($C!=""?'ZmÄ›nit trigger'.": ".h($C):'VytvoÅ™it trigger'),$n,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>ÄŒas<td>',html_select("Timing",$oi["Timing"],$J["Timing"],"triggerChange(/^".preg_quote($a,"/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"),'<tr><th>UdÃ¡lost<td>',html_select("Event",$oi["Event"],$J["Event"],"this.form['Timing'].onchange();"),(in_array("UPDATE OF",$oi["Event"])?" <input name='Of' value='".h($J["Of"])."' class='hidden'>":""),'<tr><th>Typ<td>',html_select("Type",$oi["Type"],$J["Type"]),'</table>
<p>NÃ¡zev: <input name="Trigger" value="',h($J["Trigger"]),'" maxlength="64" autocapitalize="off">
...
as$Ze=>$Ig){if(preg_match('~^(.+)(\\(.*\\))?$~U',$Ze,$B))grant("REVOKE",array_keys($Ig),$B[2]," ON $B[1] FROM $Te");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'UÅ¾ivatel byl zmÄ›nÄ›n.':'UÅ¾ivatel byl vytvoÅ™en.'),!$n);if($Fb)$f->query("DROP USER $Te");}}page_header((isset($_GET["host"])?'UÅ¾ivatel'.": ".h("$ha@$_GET[host]"):'VytvoÅ™it uÅ¾ivatele'),$n,array("privileges"=>array('','OprÃ¡vnÄ›nÃ­')));if($_POST){$J=$_POST;$kd=$Re;}else{$J=$_GET+array("host"=>$f->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$J["pass"]=$ef;if($ef!="")$J["hashed"]=true;$kd[(DB==""||$kd?"":idf_escape(addcslashes(DB,"%_\\"))).".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
<tr><th>Server<td><input name="host" maxlength="60" value="',h($J["host"]),'" autocapitalize="off">
<tr><th>UÅ¾ivatel<td><input name="user" maxlength="16" value="',h($J["user"]),'" autocapitalize="off">
<tr><th>Heslo<td><input name="pass" id="pass" value="',h($J["pass"]),'" autocomplete="new-password">
';if(!$J["hashed"])echo
script("typePassword(qs('#pass'));");echo
checkbox("hashed",1,$J["hashed"],'ZahaÅ¡ovanÃ©',"typePassword(this.form['pass'], this.checked);"),'</table>

';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".'OprÃ¡vnÄ›nÃ­'.doc_link(array('sql'=>"grant.html#priv_level"));$s=0;foreach($kd
as$Ze=>$jd){echo'<th>'.($Ze!="*.*"?"<input name='objects[$s]' value='".h($Ze)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$s]' value='*.*' size='10'>*.*");$s++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'DatabÃ¡ze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$Ab=>$Vb){foreach((array)$gg[$Ab]as$fg=>$ub){echo"<tr".odd()."><td".($Vb?">$Vb<td":" colspan='2'").' lang="en" title="'.h($ub).'">'.h($fg);$s=0;foreach($kd
as$Ze=>$jd){$C="'grants[$s][".h(strtoupper($fg))."]'";$Y=$jd[strtoupper($fg)];if($Ab=="Server Admin"&&$Ze!=(isset($kd["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".'Povolit'."<option value='0'".($Y=="0"?" selected":"").">".'ZakÃ¡zat'."</select>";else{echo"<td align='center'><label class='block'>","<input type='checkbox' name=$C value='1'".($Y?" checked":"").($fg=="All privileges"?" id='grants-$s-all'>":">".($fg=="Grant option"?"":script("qsl('input').onclick = function () { if (this.checked) formUncheck('grants-$s-all'); };"))),"</label>";}$s++;}}}echo"</table>\n",'<p>
<input type="submit" value="UloÅ¾it">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstranit">',confirm(sprintf('Odstranit %s?',"$ha@$_GET[host]"));}echo'<input type="hidden" name="token" value="',$di,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$n){$de=0;foreach((array)$_POST["kill"]as$X){if(kill_process($X))$de++;}queries_redirect(ME."processlist=",lang(array('Byl ukonÄen %d proces.','Byly ukonÄeny %d procesy.','Bylo ukonÄeno %d procesÅ¯.'),$de),$de||!$_POST["kill"]);}page_header('Seznam procesÅ¯',$n);echo'
<form action="" method="post">
...
as$y=>$X)echo"<td>".(($x=="sql"&&$y=="Info"&&preg_match("~Query|Killed~",$J["Command"])&&$X!="")||($x=="pgsql"&&$y=="current_query"&&$X!="<IDLE>")||($x=="oracle"&&$y=="sql_text"&&$X!="")?"<code class='jush-$x'>".shorten_utf8($X,100,"</code>").' <a href="'.h(ME.($J["db"]!=""?"db=".urlencode($J["db"])."&":"")."sql=".urlencode($X)).'">'.'Klonovat'.'</a>':nbsp($X));echo"\n";}echo'</table>
<p>
';if(support("kill")){echo($s+1)."/".sprintf('%d celkem',max_connections()),"<p><input type='submit' value='".'UkonÄit'."'>\n";}echo'<input type="hidden" name="token" value="',$di,'">
</form>
...
as$Xd=>$W)$_.=where_link($s++,$Xd,$W);}$X=select_value($X,$_,$o,$Sh);$t=h("val[$wi][".bracket_escape($y)."]");$Y=$_POST["val"][$wi][bracket_escape($y)];$mc=!is_array($J[$y])&&is_utf8($X)&&$K[$Pe][$y]==$J[$y]&&!$id[$y];$Rh=preg_match('~text|lob~',$o["type"]);if(($_GET["modify"]&&$mc)||$Y!==null){$pd=h($Y!==null?$Y:$J[$y]);echo"<td>".($Rh?"<textarea name='$t' cols='30' rows='".(substr_count($J[$y],"\n")+1)."'>$pd</textarea>":"<input name='$t' value='$pd' size='$oe[$y]'>");}else{$se=strpos($X,"<i>...</i>");echo"<td id='$t' data-text='".($se?2:($Rh?1:0))."'".($mc?"":" data-warning='".h('Ke zmÄ›nÄ› tÃ©to hodnoty pouÅ¾ijte odkaz upravit.')."'").">$X</td>";}}}if($Pa)echo"<td>";$b->backwardKeysPrint($Pa,$K[$Pe]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n";}if(!is_ajax()){if($K||$E){$Ac=true;if($_GET["page"]!="last"){if($z==""||(count($K)<$z&&($K||!$E)))$fd=($E?$E*$z:0)+count($K);elseif($x!="sql"||!$Sd){$fd=($Sd?false:found_rows($S,$Z));if($fd<max(1e4,2*($E+1)*$z))$fd=reset(slow_query(count_rows($a,$Z,$Sd,$ld)));else$Ac=false;}}$Df=($z!=""&&($fd===false||$fd>$z||$E));if($Df){echo(($fd===false?count($K)+1:$fd-$E*$z)>$z?'<p><a href="'.h(remove_from_uri("page")."&page=".($E+1)).'" class="loadmore">'.'NahrÃ¡t dalÅ¡Ã­ data'.'</a>'.script("qsl('a').onclick = partial(selectLoadMore, ".(+$z).", '".'NahrÃ¡vÃ¡ se'."...');",""):''),"\n";}}echo"<div class='footer'><div>\n";if($K||$E){if($Df){$_e=($fd===false?$E+(count($K)>=$z?2:1):floor(($fd-1)/$z));echo"<fieldset>";if($x!="simpledb"){echo"<legend><a href='".h(remove_from_uri("page"))."'>".'StrÃ¡nka'."</a></legend>",script("qsl('a').onclick = function () { pageClick(this.href, +prompt('".'StrÃ¡nka'."', '".($E+1)."')); return false; };"),pagination(0,$E).($E>5?" ...":"");for($s=max(1,$E-4);$s<min($_e,$E+5);$s++)echo
pagination($s,$E);if($_e>0){echo($E+5<$_e?" ...":""),($Ac&&$fd!==false?pagination($_e,$E):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$_e'>".'poslednÃ­'."</a>");}}else{echo"<legend>".'StrÃ¡nka'."</legend>",pagination(0,$E).($E>1?" ...":""),($E?pagination($E,$E):""),($_e>$E?pagination($E+1,$E).($_e>$E+1?" ...":""):"");}echo"</fieldset>\n";}echo"<fieldset>","<legend>".'CelÃ½ vÃ½sledek'."</legend>";$ac=($Ac?"":"~ ").$fd;echo
checkbox("all",1,0,($fd!==false?($Ac?"":"~ ").lang(array('%d Å™Ã¡dek','%d Å™Ã¡dky','%d Å™Ã¡dkÅ¯'),$fd):""),"var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$ac' : checked); selectCount('selected2', this.checked || !checked ? '$ac' : checked);")."\n","</fieldset>\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>ZmÄ›nit</legend><div>
...
as$y=>$X)json_row("sum-$y",format_number($X));json_row("");}elseif($_GET["script"]=="kill")$f->query("KILL ".number($_POST["kill"]));else{foreach(count_tables($b->databases())as$l=>$X){json_row("tables-$l",$X);json_row("size-$l",db_size($l));}json_row("");}exit;}else{$Kh=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($Kh&&!$n&&!$_POST["search"]){$H=true;$Ee="";if($x=="sql"&&$_POST["tables"]&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$H=truncate_tables($_POST["tables"]);$Ee='Tabulky byly vyprÃ¡zdnÄ›ny.';}elseif($_POST["move"]){$H=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$Ee='Tabulky byly pÅ™esunuty.';}elseif($_POST["copy"]){$H=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$Ee='Tabulky byly zkopÃ­rovÃ¡ny.';}elseif($_POST["drop"]){if($_POST["views"])$H=drop_views($_POST["views"]);if($H&&$_POST["tables"])$H=drop_tables($_POST["tables"]);$Ee='Tabulky byly odstranÄ›ny.';}elseif($x!="sql"){$H=($x=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$Ee='Tabulky byly optimalizovÃ¡ny.';}elseif(!$_POST["tables"])$Ee='Å½Ã¡dnÃ© tabulky.';elseif($H=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($J=$H->fetch_assoc())$Ee.="<b>".h($J["Table"])."</b>: ".h($J["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$Ee,$H);}page_header(($_GET["ns"]==""?'DatabÃ¡ze'.": ".h(DB):'SchÃ©ma'.": ".h($_GET["ns"])),$n,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3 id='tables-views'>".'Tabulky a pohledy'."</h3>\n";$Jh=tables_list();if(!$Jh)echo"<p class='message'>".'Å½Ã¡dnÃ© tabulky.'."\n";else{echo"<form action='' method='post'>\n";if(support("table")){echo"<fieldset><legend>".'Vyhledat data v tabulkÃ¡ch'." <span id='selected2'></span></legend><div>","<input type='search' name='query' value='".h($_POST["query"])."'>",script("qsl('input').onkeydown = partialArg(bodyKeydown, 'search');","")," <input type='submit' name='search' value='".'Vyhledat'."'>\n","</div></fieldset>\n";if($_POST["search"]&&$_POST["query"]!=""){$_GET["where"][0]["op"]="LIKE %%";search_tables();}}$bc=doc_link(array('sql'=>'show-table-status.html'));echo"<table cellspacing='0' class='nowrap checkable'>\n",script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});"),'<thead><tr class="wrap">','<td><input id="check-all" type="checkbox" class="jsonly">'.script("qs('#check-all').onclick = partial(formCheck, /^(tables|views)\[/);",""),'<th>'.'Tabulka','<td>'.'ÃšloÅ¾iÅ¡tÄ›'.doc_link(array('sql'=>'storage-engines.html')),'<td>'.'PorovnÃ¡vÃ¡nÃ­'.doc_link(array('sql'=>'charset-charsets.html','mariadb'=>'supported-character-sets-and-collations/')),'<td>'.'Velikost dat'.$bc,'<td>'.'Velikost indexÅ¯'.$bc,'<td>'.'VolnÃ© mÃ­sto'.$bc,'<td>'.'Auto Increment'.doc_link(array('sql'=>'example-auto-increment.html','mariadb'=>'auto_increment/')),'<td>'.'Å˜Ã¡dkÅ¯'.$bc,(support("comment")?'<td>'.'KomentÃ¡Å™'.$bc:''),"</thead>\n";$T=0;foreach($Jh
as$C=>$U){$Oi=($U!==null&&!preg_match('~table~i',$U));$t=h("Table-".$C);echo'<tr'.odd().'><td>'.checkbox(($Oi?"views[]":"tables[]"),$C,in_array($C,$Kh,true),"","","",$t),'<th>'.(support("table")||support("indexes")?"<a href='".h(ME)."table=".urlencode($C)."' title='".'Zobrazit strukturu'."' id='$t'>".h($C).'</a>':h($C));if($Oi){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.'PozmÄ›nit pohled'.'">'.(preg_match('~materialized~i',$U)?'MaterializovanÃ½ pohled':'Pohled').'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.'Vypsat data'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'PozmÄ›nit tabulku'),"Index_length"=>array("indexes",'PozmÄ›nit indexy'),"Data_free"=>array("edit",'NovÃ¡ poloÅ¾ka'),"Auto_increment"=>array("auto_increment=1&create",'PozmÄ›nit tabulku'),"Rows"=>array("select",'Vypsat data'),)as$y=>$_){$t=" id='$y-".h($C)."'";echo($_?"<td align='right'>".(support("table")||$y=="Rows"||(support("indexes")&&$y!="Data_length")?"<a href='".h(ME."$_[0]=").urlencode($C)."'$t title='$_[1]'>?</a>":"<span$t>?</span>"):"<td id='$y-".h($C)."'>&nbsp;");}$T++;}echo(support("comment")?"<td id='Comment-".h($C)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d celkem',count($Jh)),"<td>".nbsp($x=="sql"?$f->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$y)echo"<td align='right' id='sum-$y'>&nbsp;";echo"</table>\n";if(!information_schema(DB)){echo"<div class='footer'><div>\n";$Ii="<input type='submit' value='".'VyÄistit'."'> ".on_help("'VACUUM'");$nf="<input type='submit' name='optimize' value='".'Optimalizovat'."'> ".on_help($x=="sql"?"'OPTIMIZE TABLE'":"'VACUUM OPTIMIZE'");echo"<fieldset><legend>".'OznaÄenÃ©'." <span id='selected'></span></legend><div>".($x=="sqlite"?$Ii:($x=="pgsql"?$Ii.$nf:($x=="sql"?"<input type='submit' value='".'Analyzovat'."'> ".on_help("'ANALYZE TABLE'").$nf."<input type='submit' name='check' value='".'Zkontrolovat'."'> ".on_help("'CHECK TABLE'")."<input type='submit' name='repair' value='".'Opravit'."'> ".on_help("'REPAIR TABLE'"):"")))."<input type='submit' name='truncate' value='".'VyprÃ¡zdnit'."'> ".on_help($x=="sqlite"?"'DELETE'":"'TRUNCATE".($x=="pgsql"?"'":" TABLE'")).confirm()."<input type='submit' name='drop' value='".'Odstranit'."'>".on_help("'DROP TABLE'").confirm()."\n";$k=(support("scheme")?$b->schemas():$b->databases());if(count($k)!=1&&$x!="sqlite"){$l=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'PÅ™esunout do jinÃ© databÃ¡ze'.": ",($k?html_select("target",$k,$l):'<input name="target" value="'.h($l).'" autocapitalize="off">')," <input type='submit' name='move' value='".'PÅ™esunout'."'>",(support("copy")?" <input type='submit' name='copy' value='".'ZkopÃ­rovat'."'>":""),"\n";}echo"<input type='hidden' name='all' value=''>";echo
script("qsl('input').onclick = function () { selectCount('selected', formChecked(this, /^(tables|views)\[/));".(support("table")?" selectCount('selected2', formChecked(this, /^tables\[/) || $T);":"")." }"),"<input type='hidden' name='token' value='$di'>\n","</div></fieldset>\n","</div></div>\n";}echo"</form>\n",script("tableCheck();");}echo'<p class="links"><a href="'.h(ME).'create=">'.'VytvoÅ™it tabulku'."</a>\n",(support("view")?'<a href="'.h(ME).'view=">'.'VytvoÅ™it pohled'."</a>\n":"");if(support("routine")){echo"<h3 id='routines'>".'Procedury a funkce'."</h3>\n";$Ng=routines();if($Ng){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'NÃ¡zev'.'<td>'.'Typ'.'<td>'.'NÃ¡vratovÃ½ typ'."<td>&nbsp;</thead>\n";odd('');foreach($Ng
as$J){$C=($J["SPECIFIC_NAME"]==$J["ROUTINE_NAME"]?"":"&name=".urlencode($J["ROUTINE_NAME"]));echo'<tr'.odd().'>','<th><a href="'.h(ME.($J["ROUTINE_TYPE"]!="PROCEDURE"?'callf=':'call=').urlencode($J["SPECIFIC_NAME"]).$C).'">'.h($J["ROUTINE_NAME"]).'</a>','<td>'.h($J["ROUTINE_TYPE"]),'<td>'.h($J["DTD_IDENTIFIER"]),'<td><a href="'.h(ME.($J["ROUTINE_TYPE"]!="PROCEDURE"?'function=':'procedure=').urlencode($J["SPECIFIC_NAME"]).$C).'">'.'ZmÄ›nit'."</a>";}echo"</table>\n";}echo'<p class="links">'.(support("procedure")?'<a href="'.h(ME).'procedure=">'.'VytvoÅ™it proceduru'.'</a>':'').'<a href="'.h(ME).'function=">'.'VytvoÅ™it funkci'."</a>\n";}if(support("sequence")){echo"<h3 id='sequences'>".'Sekvence'."</h3>\n";$bh=get_vals("SELECT sequence_name FROM information_schema.sequences WHERE sequence_schema = current_schema() ORDER BY sequence_name");if($bh){echo"<table cellspacing='0'>\n","<thead><tr><th>".'NÃ¡zev'."</thead>\n";odd('');foreach($bh
...
as$J){echo"<tr>","<th>".h($J["Name"]),"<td>".($J["Execute at"]?'V danÃ½ Äas'."<td>".$J["Execute at"]:'KaÅ¾dÃ½ch'." ".$J["Interval value"]." ".$J["Interval field"]."<td>$J[Starts]"),"<td>$J[Ends]",'<td><a href="'.h(ME).'event='.urlencode($J["Name"]).'">'.'ZmÄ›nit'.'</a>';}echo"</table>\n";$zc=$f->result("SELECT @@event_scheduler");if($zc&&$zc!="ON")echo"<p class='error'><code class='jush-sqlset'>event_scheduler</code>: ".h($zc)."\n";}echo'<p class="links"><a href="'.h(ME).'event=">'.'VytvoÅ™it udÃ¡lost'."</a>\n";}if($Jh)echo
